...
Slide 19: A security label in military context constst of a level (left side on slide 18) and a compartment (right side). We create labels as combinations of a level and a set of keywords. You order them first pointwise and then for the compartments you order htem by subset inclusions. 
Slide 20: Example. Confidential info about A (C,{A}) is less confidential than secret info about A (S,{A}). This existed in the miliatry before computers.
Slide 21: Script-security for webapplications: we had the same-origin policy: a lattice that compartmentalizes websites as shown on the picture. 
Slide 22: A security automaton. We'l use the notion of security labels and in addition we'll introduce the notion of a subject or a session that the user can start. In web context as a session, in OS context as a specific login.
We need different subjects or sessions will become clear later. The guard will have a notion of session which represents 1 specific interaction between a suer and a part of the system. 
They will be labeled with a security label on creaton. If a session gets label L, it can only ever see information with level L or lower.
If you have someone with a high clearance, he/she may want to work in different modes of operation: may want to work with top0secret stuff, but also with secret or public information. If you don't have this, there are a lot of limitations on what high-level users can do.
If you're logged in at confidential, you can read public objects, but not top-secret objects.
*-property: if you log in at confidential level, you can only write on confidential level and not on public level (but you can read on public level). This is sto stop the trojan horse program. The OS will enforce that a malicious program can not write top-secret information to a pulic file for example. This is shown in slide 23.
Log in at secret level: read file F and F', but not write to File F' (but can write File F).
This is why we need multiple subjects: if any person with high clearance, if he could only log in with his high clearance, he would only be able to read and write at his level. We need multiple subjects to make things workable for people with high lcearance.
Slide 24: automaton for LBAC. 
The dynamic part of the protection state can change while running. You have objects and sessions. They will be labeled with slabel and olabel. Read $\&$ write are simple: if a session wants to read an object, it can do so if its slabel >= olabel.
...
Even at this level of abstraction, there are a number of known exploits that can still pass information to lower levels: modulate the load of the CPU: written such that you let it write a 1 or 0 and this can be read by S2. it's ipossible to make a useable system that doesn't have these side-channels.
It's very rarely used these days, it is used for integrity protection. 
Slide 27: RBAC is used in many applications.
Slide 28: It's simpler than LBAC. The main objective is to mae access control manageable. DAC: users can do stupid things and it's hard to manage. (??/ 1.02.00).
The notion of a professor f.e. is more stable than any individual user. A role can be a sepcific job/responsibility.you need a fairly stable set of permissions: the permissions of a professor are more seady than those of individual users. There's someone who can decide what kind of righsta a professor can have. When he leaves, the rights are deleted. 
We'll discuss a variant where users can start multiple sessions with different rights. I, as a user, can choose which of my roles I want to activate. When a session si started, yo uhave to choose which role needs to be activated. When logged in as both, you'll have the union of the rights.
Slide 29: sessions can be started and stopped. Users can start multiple sessions
checkAccess: the permissions should be given to one of the roles that is activated in the current session. There should exist a role that has that specific permission. If tha'ts the case, it's okay, otherwise it isn't. 
Slide 30: createSession: the roles that I'm asking for should be a subset of th eroles assigned to this user. 
...
Slide 31: any permission associated with Project B Eng automatically also has all permissions of Engineering Dept. you can have specific permissions associated with the projects and then you have the director that has access to everything. You put the permissions that are common to every engineer and then the subsets of users that can do more become smaller.
Slide 32: other extensions: put constrtaitns on roles. Fraud is much less likely to happen if 2 people need to collaborate. Having 2 people responsible for something, decreases the chances of people doing bad stuff --> static separation of duty. One role can order goods and one role can aprove payment and no user can be both roles. 
Dynamic constrains: (??? 1.13.30) principle of least privilege: when doing something, make sure you have the least permissions necessary to perform that job. 
Slide 34: LBAC for integrity: you control the flow of information. Info can not flow from secret to public. For integrity, that's also flow of info. Protecting the integrity of your files is the same as saying tha tno ino from the internet should flow to that file. You can have labels of trustworthiness instead of labels of ( 1.15.00?). Windows uses this. 
More dynamic access control models (? 1.16.30). The atct of payment changes the permissions you have. You do something in the system (a normal activity) and the protection state changes because of that activity. It's easy to model with security automata.
Exam question: come up with a sensible (? 1.18.00).
Slide 36: Security automata are specification- (1.18.30).
...
Slide 38: 4 big approaches, 1 slide per approach.
Rely on the OS: reuse the access control that the OS already has. You ask the OS to authenticate the user and then the application starts a thread in the name of the authenticated user and runs it. Anything the thread does, the OS will assume it's that user. If it tries to do something it's not allowed to do, the OS will complain.
Slide 39: Use application servers. Kind of an OS for application. They offer a number of services that many applications will need and one of these servers can be access control. Anybody who wants to do an operation onyour application has to pass throught the container. You implement access checks in the red line. F.e. if you want to change the price of a product somewhere, this can only be done if the user is on the OS. You reuse access control that has been implemented by someone else, but on a different level than in the previous slide.
Slide 40: Specific helper software: reverse proxies: a machine you put in between the applications and the users of the applications. On the PEP-level you implement access control. PEP = ,PDP = (policy decision point denk ik) 1.23.00. this way, the application doesn't have to do access control.
Slide 41: Do it in the application itself. Avoid this because it's very hard to have complete mediation here. It's easy to forget access checks. Wherever you're manipulating things in the application code, you'll have to think of an explicit acces-ceck. This is known to be very errorprone. It's a very flexible approach but it's very hard to evolve and very easy to make mistakes. 
You can have the checks all out to a PDP: you would collect all the needed info and pass that on to an authorization engine so that at leas thte policy can evolve rather easily, without having to change the source code.
Slide 42: This is one of the most important mechanisms for application-level security. Low0level and webapp issues are the number 1 vulnerabilties today, but bad access control is definitely also in the top 3 or 5 of why software security fails. The reason why it's error-prone because it's easy to get policies wrong and because they're hard to implement (especially complete mediation).
Whenever you use discretionary policies (1.27.45).
