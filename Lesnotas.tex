...
\paragraph{Slide 41:} The list of mitre is very specific. It identifies a specific vulnerability in a specific version of a specific system. The CWE tries to abstract a a bit: they try to classify them according to types and then (??).
...
\paragraph{Slide 44:} Typically, on a heavy web application, you have (… 19.00). There will be code running on the application server. You can write code that dynamically constructs SQL-commands to communicate with the back-end. In sQL-injection you manipulate strings. The given example can be used to check if the user with the given password is registrated. If all goes well, then you get the query in slide 46. the result will contain 1 entry. Otherwise it will send you to a page to create an account. The code is insufficiently defensive.
...
\paragraph{Slide 54:} Prevalence of vulnerabilities. The blue line seems here to stay. Some vulnerabilities come up and die quickly, but are almost not exploited today. The difficult ones only die out very slowly.

\section{2. LowLevelSoftwareSecurity}
...
\item Unsfae API: might be that they are unsafe in nature, which you might then exploit. They have a variable number of arguments and you can specify a format string and that way exploit it like spatial safety errors.
\end{itemize}
...
\item Temporal: it does its own garbage collection. There are languages that try to achieve safety w/o garbage collection, but this is tricky. Something is only freed if you can assure that you can't access it anymore.
\end{itemize}

\paragraph{Slide 6:} C does all this for optimization purposes. C is designed for performance, so that's why it doesn't check bounds etc. if behaviour is undefined, it depends on the compiler: very implementation-dependend. The trick of all attacks will be to know the implementation details so well that you can tune it to your advantage.
...
\begin{itemize}
%TODO code mooi maken
\item Automatic: you can in the body of the function declare variables. E.g.
	int f () {    int I = 10;    char b[80];} space for these variables will be allocated "automatically": any invocation of f will give you different instantiations of f. when an invocation to f happens, a new part of memory will be allocated on the stack. As a programmer, you don't have to do anything to (de)allocate this. The activation record of f will be popped afterwards automatically.
\item Static: it's possible to declare variables outside the body and then you will be able to call it everywhere in the class. The compiler will, once and for all, allocate memory for that variable. You don't have to worry about allocation or freeing becaueys it lives the entire lifetime of the program. E.g.:int j = 10;int f () {    int I = 10;    char b[80];}
\item Dynamic: (C: malloc, java: new). This is the only form of explicit memory allocation. Then somehwere in memory, the library will call a piece of memory in runtime until you free it/the garbage collector takes it away. E.g.:int f () {    int I = 10;    char b[80];    int * p.malloc(10); ??} free(p);
\end{itemize}

\paragraph{Slide 10:} Division between the data is a convention maintained by the compiler.

\paragraph{Slide 11:} You can see this at work: a program with s a static global variable, a local variable (l) nad a dynamic variable (d). 
...
\paragraph{Slide 14:} We see the stack and the instruction pointer somewhere in the code (with f1: machine code for f1). F0 will call f1. on (and on top of) the stack we have an activation record of f0 (if f1 is called within f0). If f0 calls f1, a vnew activation record is added to the stack. F0 will push the arguments to the stack (\textbf{Slide 15}), then execute the call and (1.02). 
What happens when we overlfow? We do something that triggers a spatial memory safety error. The red part in \textbf{Slide 16} is overwritten. You start putting stuff at the bottom of space for buffer \textbf{Slide 15} and then you overwrite all this stuff. How do you overwrite it to be useful to you? The thing to aim for is the return address. If f1 reaches return, the implementation of the machine code will look at that place in the stack and will jump to that machine code, if you overwrite the address, you can do whatever you want. The basic one will overwrite it with an address that is in the stack itself. Because processors don't care/distinguish between data and code. You can make the processor jump anywhere you want. You can control this memory content. The stack is the input for the program. You choose the data so that it is interpreted as machine code (I think?). The instruction pionter will go to your instructions and then you can make it do whatever you want.
...
You can recognize our attack code when you see feeb2ecd: FE EB 2E CD. If the processor would be big-endian, you woul dhave to read it the other way around.
...
If we copy the first and the second string. The fst string contains 'file' and the second string is the evil one. The code will start to load the second string into the buffer. In \textbf{Slide 22} we see the tricks at work, see \textbf{Slide 23}. \\
